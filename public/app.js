...
function YouTubePlayer(props) {
  ...
  var embedUrl = 'https://www.youtube.com/embed/' + videoId + '?enablejsapi=1&autoplay=1&rel=0&modestbranding=1&origin=' + encodeURIComponent(window.location.origin);
  ...
  useEffect(function() {
    function handleMessage(event) {
      ...
      try {
        ...
        // YouTube sends info events
        if (data.event === 'infoDelivery' && data.info) {
          if (data.info.playerState !== undefined) {
            var state = data.info.playerState;
            var currentTime = data.info.currentTime || 0;
            
            // Don't report state changes we caused
            if (Date.now() - lastCommandTime.current < 500) return;
            
            // 1 = playing, 2 = paused
            if (state === 1 && playbackState !== 'playing') {
              console.log('YT: Playing at', currentTime);
              onStateChange('playing', currentTime);
            } else if (state === 2 && playbackState !== 'paused') {
              console.log('YT: Paused at', currentTime);
        ...
        if (data.event === 'onReady') {
          ...
          sendCommand('listening', ['playerState', 'currentTime']);
        }
      } catch (e) {}
    }
  ...
  function sendCommand(func, args) {
    ...
    var message = JSON.stringify({ event: 'command', func: func, args: args || [] });
    iframeRef.current.contentWindow.postMessage(message, 'https://www.youtube.com');
  }

  // Apply remote state changes
  useEffect(function() {
    if (isLocalChange || !initialized.current) return;
    
    if (playbackState === 'playing') {
      console.log('Sending playVideo command');
      sendCommand('playVideo');
    } else if (playbackState === 'paused') {
      console.log('Sending pauseVideo command');
      sendCommand('pauseVideo');
    }
    
    if (playbackTime > 0) {
      sendCommand('seekTo', [playbackTime, true]);
    }
